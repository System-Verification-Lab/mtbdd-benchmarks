...
QSY_QASM = "timeout {} ./tools/q-sylvan/build/qasm/run_qasm_on_{} {} --workers {} {} --json {} &> {}\n"
...
parser.add_argument('--tools', nargs='+', default=['q-sylvan'], help="Which tools to include (q-sylvan, mqt, quasimodo).")
parser.add_argument('--workers', nargs='+', default=[1], type=int, help="Run multicore benchmarks for given number of workers.")
parser.add_argument('--timeout', action='store', default='600m', help='Timeout time per benchmark')
...
def experiments_sim_qasm(args):
    ...
    with open(bash_file_all, 'w', encoding='utf-8') as f_all,\
        ...
        for filepath in sorted(filepaths, key=natural_sorting):
            ...
            # Q-Sylvan (QMDD)
            if 'q-sylvan' in args.tools and 'evdd' in args.dd_types:
                    ...
                    for s in norm_strats:
                        for inv in inv_caching:
                            for r in reorder:
                                ...
                                f_all.write(QSY_QASM.format(args.timeout, 'qmdd', filepath, w, qsy_args+s+inv+r, json_output, log))
                                f_qsy.write(QSY_QASM.format(args.timeout, 'qmdd', filepath, w, qsy_args+s+inv+r, json_output, log))
                                with open(meta, 'w', encoding='utf-8') as meta_file:
                                    json.dump({ 'circuit_type' : filename.split('_')[0],
                                                'circuit' : filename[:-5],
                                                'dd_type' : 'evdd',
            ...
            if 'q-sylvan' in args.tools and 'mtbdd' in args.dd_types:
                # ground truth run
                if args.ground_truth_prec != 0:
                    prec = args.ground_truth_prec
                    w = 1
                    tol = 0
                    exp_counter += 1
                    json_output = f"{output_dir}/json/{filename[:-5]}_qsylvan_{exp_counter}.json"
